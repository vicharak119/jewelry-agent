...
export async function onRequest(ctx) {
  ...
  try {
    ...
    if (method === "POST" && path === "analyze") {
      ...
      // Selected placements (1-3). One analysis returns a DISTINCT scene set per placement.
      var displays = (fd.get("displays") || "").toString().split(",").map(function (s) { return s.trim(); }).filter(Boolean);
      if (!displays.length) displays = ["mannequin"];
      var dispLabel = function (d) { return d === "body" ? "worn on a model (only the relevant body area, modest framing)" : d === "props" ? "arranged on themed props (no mannequin or body in frame)" : "on a clean jewelry bust/stand"; };
      var scenesRule = "The user will create " + displays.length + " marketing image" + (displays.length > 1 ? "s" : "") + ", one per placement, in THIS exact order: " + displays.map(function (d, i) { return (i + 1) + ") " + dispLabel(d); }).join("; ") + ". Return a \"scenes\" array with EXACTLY " + displays.length + " entr" + (displays.length > 1 ? "ies" : "y") + " IN THE SAME ORDER. Each scene must be DISTINCT from the others" + (displays.length > 1 ? " — clearly different background, " + (wantProps ? "different props, " : "") + "different arrangement so the images do NOT look like the same scene repeated" : "") + ", each still suiting the mood and complementing the product's colours.";
      ...
      var ar = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "gpt-4o", max_tokens: 1000, messages: [
          { role: "system", content: 'You are a luxury jewelry photography director. Analyze the jewelry with EXTREME precision - every charm, stone, bead, chain link pattern matters. IMPORTANT: a photo may contain MORE THAN ONE piece (for example a necklace AND matching earrings AND a ring). Identify EVERY distinct piece you can see, including small ones like earrings. You ALSO design the marketing scene: the background, props and colour mood you return are part of the creative brief and must COMPLEMENT the product\'s real metal and gemstone colours (warm staging for gold, cooler for silver/white-gold, accent props echoing the gem colour). Always return a styled background that complements the product; FOLLOW THE USER\'S PROPS INSTRUCTION in the user message (it decides whether props are included or empty). Return ONLY valid JSON: {"items":["short name of each distinct piece, e.g. \'gold floral necklace\', \'matching drop earrings\'"],"type":"the primary/most prominent piece type","metal":"metal/finish","elements":"VERY detailed description across ALL pieces - count of charms, exact shapes, colors, placement, spacing","style":"design style","chain_type":"chain type","display":"recommended mannequin/display","background":"a styled marketing background/surface matching the chosen style and complementing the product colours","props":"specific decorative props (comma separated) that suit the style AND the product colours, OR an empty string if the user asked for no props","color_mood":"palette tying the product colours to the scene","scenes":[{"placement":"which placement this scene is for","background":"styled background invented for this placement","props":"props (comma separated) for this placement, or empty if no-props","color_mood":"palette for this placement"}]}' },
